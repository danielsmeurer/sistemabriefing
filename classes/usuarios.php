...
Class usuarios{
	...
	*/
	 ...
	 * @param array $usuario
	 * return array com mensagens de validação ou de sucesso
	 */	
	function criar($usuario){
		...
		if($this->user['login']==''){ 
			$msg['error']['login']="O preenchimento do campo Login é obrigatório.";
		}
		elseif(strlen($this->user['login'])<4){
			...
			if($login_existe){ $msg['error']['login']="Este login já existe em nosso cadastro";}
		}
		...
		if($this->user['conf_senha']!==$this->user['senha']){
			$msg['error']['conf_senha']="O confirmação de Senha deve ser igual ao campo Senha.";
		}
	...
	function editar($usuario=false){
		...
		if(!isset($usuario['nome']) or empty($usuario['nome']) ){
			$usuario['erro']['nome']= "O campo Nome é obrigatório";
		}
		if(!isset($usuario['login']) or empty($usuario['login']) ){
			$usuario['erro']['login']= "O campo Login é obrigatório";
		}
		
		if(isset($usuario['new_senha'])){
			if(!($usuario['new_senha']) or (empty($usuario['new_senha'])) ){
				$usuario['erro']['new_senha']= "O campo Nova senha é obrigatório";
			}
			...
			if(!($usuario['conf_new_senha']) or (empty($usuario['conf_new_senha'])) ){
				$usuario['erro']['conf_new_senha']= "O campo Confirmar de nova senha é obrigatório";
			}
